...
$sql3 = "SELECT task_id, task_deadline, task_status, task_recipient_user_id, task_name, task_recipient_department_id,
        ...
        WHERE task_work_request_id = wro1.work_request_id AND (task_status = 'Draft' )";
...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Lato:ital,wght@0,100;0,300;0,400;0,700;0,900;1,100;1,300;1,400;1,700;1,900&family=Noto+Sans+Thai:wght@100..900&display=swap"
        rel="stylesheet">
    ...
    <?php if (!empty($data3)): ?>
    <!-- แสดงผลลัพธ์ถ้ามีข้อมูลใน $data3 -->
    <p>พบข้อมูลงานย่อยจำนวน <?= count($data3) ?> รายการ</p>


    <?php else: ?>
    <!-- แสดงข้อความถ้าไม่มีข้อมูล -->
    <p>ไม่พบข้อมูลงานย่อย</p>
    <?php endif; ?>
</head>

<body class="bg-gray-100 text-gray-900" x-data="{ isOpen: false, draftModal: false, current_work: null }">
    <!-- เริ่มส่วน Sidebar -->
                ...
                <div class="overflow-y-auto h-[500px] pr-2 scrollbar-hide">
                    <div class="mt-4 grid grid-cols-2 gap-4">
                        ...
                        <div class="p-4 rounded-lg h-[140px] relative shadow" style="background-color: #EEFFE5;">
                            <!-- เสร็จสิ้น badge -->
                            <span
                                class="absolute top-4 right-4 text-[10px] px-2 py-0.5 rounded-full border border-dashed border-[#6E44FF] text-[#6E44FF] bg-white"
                            ...
                            <p class="font-semibold text-[16px] text-[#3A3541] truncate">
                                <?= htmlspecialchars($row['work_name']) ?> </p>

                            <!-- ผู้ใช้ -->
                            <?php if ($row['work_author_type'] === 'P'): ?>
                            <p class="text-[13px] text-[#3A3541] mt-1"><?= htmlspecialchars($row['user_fname']) ?>
                                <?= htmlspecialchars($row['user_lname']) ?></p>
                            <?php elseif ($row['work_author_type'] === 'D'): ?>
                            <p class="text-[13px] text-[#3A3541] mt-1"><?= htmlspecialchars($row['department_name']) ?>
                            </p>
                            <?php endif; ?>
                            ...
                            <!-- ปุ่มตกลง -->
                            <button
                                class="absolute bottom-2 left-4 px-2 py-0.5 text-[10px] border border-black rounded-full bg-white text-black hover:bg-black hover:text-white transition-all duration-500 ease-in-out"
                                style="border-radius: 8px">
                                ตกลง
                            </button>
                        </div>
                        ...
                        <div class="p-4 rounded-lg h-[140px] relative shadow" style="background-color: #FFF3F3;">
                            <!-- เสร็จสิ้น badge -->
                            <span
                                class="absolute top-4 right-4 text-[10px] px-2 py-0.5  rounded-full border border-dashed border-[#E60000] text-[#E60000] bg-[#ffB6B6]"
                                style="border-radius: 8px">
                                ปฏิเสธ
                            </span>
                ...
                <div class="overflow-y-auto h-[500px] pr-2 scrollbar-hide">
                    <div class="mt-4 grid grid-cols-2 gap-4">
                        ...
                        <div class="p-4 bg-white rounded-lg h-[105px] relative shadow">
                            ...
                            <span
                                class="absolute top-2 right-2 text-xs px-2 py-0.5 rounded-full border border-dashed border-[#FBBC05] text-[#FBBC05] bg-[#FFF5D8]"
                                style="border-radius: 8px">
                                กำลังดำเนินการ
                            </span>
            ...
            <div class="col-span-9 bg-white p-6 rounded-lg shadow-md h-[506px]">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-semibold">ประวัติ
                        <span
                            class="text-gray-500 text-sm font-normal">ใบสั่งงานที่ดำเนินการเสร็จสิ้นและถูกยกเลิก</span>
                    </h2>
                    <div style="margin-left: 380px;">
                        <?php if ($total_item==0): ?>
                        <p class="text-gray-500 text-sm font-normal">0 - 0 จาก 0</p>
                        <?php elseif ($total_item<=50): ?>
                        <p class="text-gray-500 text-sm font-normal">{{ $current_page }}-{{ $total_item }} จาก
                            {{ $total_item }}</p>
                        <?php elseif ($total_item<$current_page*50): ?>
                        <p class="text-gray-500 text-sm font-normal">
                            {{ ($current_page - 1) * 50 + 1 }}-{{ $total_item }}
                            จาก {{ $total_item }}</p>
                        <?php else : ?>
                        <p class="text-gray-500 text-sm font-normal">
                            {{ ($current_page - 1) * 50 + 1 }}-{{ $current_page * 50 }} จาก {{ $total_item }}</p>
                        <?php endif; ?>
                    </div>
                    <div class="button-container" style="margin-top: -10px;">
                        <button onclick="location.href='?page=<?= max(1, $current_page - 1) ?>'" class="px-4 py-2"
                            style="background-image: url('public/asset/l.png'); background-repeat: no-repeat; background-position: center;"
                            <?= $current_page === 1 ? 'disabled' : '' ?>></button>
                        <button onclick="location.href='?page=<?= min($total_pages, $current_page + 1) ?>'"
                            class="px-4 py-2"
                            style="background-image: url('public/asset/r.png'); background-repeat: no-repeat; background-position: center;"
                            <?= $current_page === $total_pages ? 'disabled' : '' ?>></button>
                    </div>
            ...
            <div class="col-span-3 bg-white p-6 rounded-lg shadow-md h-[506px]">
                ...
                <div class="overflow-y-auto h-[440px] mt-4 pr-1 scrollbar-hide space-y-3">
                    ...
                    <div class="p-4 bg-white rounded-lg shadow relative"
                        @click="draftModal = true; current_work = {
                            id: '<?= $row['work_request_id'] ?>',
                            name: '<?= $row['work_name'] ?>',
                            createDate: '<?= $row['work_create_date'] ?>',
                            submitDate: '<?= $row['work_submit_date'] ?>',
                            userId: '<?= $row['work_create_by_user_id'] ?>',
                            authorType :'<?= $row['work_author_type'] ?>'
                            }">
                        <!-- Badge -->
                        <span
                            class="absolute top-2 right-2 text-xs px-2 py-0.5 rounded-full border border-dashed border-gray-300 text-gray-500 bg-gray-100"
                            style="border-radius: 8px">
                            แบบร่าง
                        </span>
    ...
    <!-- Draft Modal test-->
    <!-- Modal -->
    <div x-show="draftModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-data="{
        tasks: [{ id: 1 , name: '', description: '' }],
        addTask() {
            this.tasks.push({ id: this.tasks.length + 1, name: '', description: '' });
        },
        addTask2(task_name,task_deadline) {
            this.tasks.push({ id: this.tasks.length + 1, name: task_name, description: task_deadline });
        },
        removeTask(index) {
        ...
        <div class="bg-white p-4 rounded-xl shadow-xl max-w-3xl p-6 relative">
            ...
            <div class="flex justify-between items-center border-b pb-2 mb-4">
                ...
                <button @click="draftModal = false , closeModal()" class="text-gray-600 hover:text-black text-xl" >
            ...
            <form action="{{ url('/workrequest') }}" method="POST">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm text-gray-800 border-b pb-3 mb-4">
                    <div>
                        <label for="work_name" class="font-semibold">ชื่องงงง :</label>
                        <input type="text" name="work_name" id="work_name"
                            class="border px-3 py-2 rounded w-half" placeholder="กรุณากรอกชื่อเรื่อง"
                            x-model ="current_work.name" required>
                    </div>
                    <div class="px-3 py-2">
                        <label class="font-semibold">วันที่ร้องขอ :</label>
                        <span name="create_date" value=""
                            class="text-gray-900 font-medium">{{ \Carbon\Carbon::now()->format('d/m/Y') }}</span>
                    </div>
                    <div>
                    ...
                    <script>
                        function author_type() {
                            return {
                                currentType: {
                                    authorType: @json(old('work_author_type', $currentItem['authorType'] ?? ''))
                                }
                            };
                        }
                    </script>

                    <div class="px-3" x-data='author_type()'>

                        <label class="font-semibold ">แผนก <span class="text-red-500">*</span> :</label>
                        <label class="space-x-2 px-3 py-3">
                            <input type="radio" name="work_author_type" value="D" class=""
                                x-model ="current_work.authorType">
                ...
                <div  class="max-h-80 overflow-y-auto scrollbar-hide">
                    ...
                    <div class="flex justify-end items-center mt-1 max-h-80 overflow-y-auto">
                        <button type="button" @click="addTask"
                            class="button-button5 bg-green-500 text-white px-4 py-1 rounded hover:bg-green-700 transition">
                            <i class="fas fa-plus"></i> เพิ่มรายการ
                        </button>
                    </div>

                    <!-- แสดงรายการงานย่อยในร่างแบบฟอร์ม -->
                    <?php foreach ($data3 as $row): ?>
                    <template x-if="current_work.id == <?= (int) $row['task_work_request_id'] ?>"
                        :key="task.id" x-data="{ task: { id: <?= (int) $row['task_id'] ?>, name: '<?= htmlspecialchars($row['task_name']) ?>', description: '' } }"  >
                        <div x-init="addTask2('{{ $row['task_name'] }}', '{{ $row['task_deadline'] }}')"></div>
                    </template>
                    <?php endforeach; ?>
                    ...
                    <template x-for="(task, index) in tasks" :key="task.id">
                        <div class="mt-4 border border-gray-300 rounded-lg p-4 space-y-3">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                <input type="text" :name="'task_name[' + index + ']'" placeholder="ชื่องาน"
                                    class="w-full border rounded px-3 py-2"  x-model= tasks[index+1].name required>
                                <div class="flex items-center border rounded px-3 py-2 w-full space-x-2">
                                    <i class="fas fa-calendar-alt text-blue-500"></i>
                                    <input type="date" :name="'task_deadline[' + index + ']'"
                                        class="flex-1 outline-none"   required>
                                </div>
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2" x-data="{ selected: 'P' }">
                                <!-- บุคคล -->
                                <label class="flex items-center border rounded px-3 py-2 space-x-2">
                                    <input type="radio" :name="'task_recipient_type[' + index + ']'" value="P"
                                        x-model="selected">
                                    <input type="text" :name="'task_recipient_user_id[' + index + ']'"
                                        placeholder="บุคคล" class="flex-1 outline-none"
                                        :disabled="selected !== 'P'" :required="selected === 'P'">
                                    <i class="fas fa-search text-gray-500"></i>
                                </label>

                                <!-- แผนก -->
                                <label class="flex items-center border rounded px-3 py-2 space-x-2">
                                    <input type="radio" :name="'task_recipient_type[' + index + ']'" value="D"
                                        x-model="selected">
                                    <input type="text" :name="'task_recipient_department_id[' + index + ']'"
                                        placeholder="แผนก" class="flex-1 outline-none"
                                        :disabled="selected !== 'D'" :required="selected === 'D'">
                                    <i class="fas fa-search text-gray-500"></i>
                                </label>
                            </div>
                ...
                <div class="flex justify-end space-x-4">
                    ...
                    <button type="submit" name="work_status" value="draft"
                        class="px-4 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-100">แบบร่าง</button>
                </div>
            </form>
        </div>
